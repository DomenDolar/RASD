create or replace package rasd_engine10x is
/*
// +----------------------------------------------------------------------+
...
';
  ...
  procedure form(p_formid rasd_forms.formid%type, p_lang rasd_fields.fieldid%type default 'SL');
...
create or replace package body rasd_engine10x is
/*
// +----------------------------------------------------------------------+
// | RASD - Rapid Application Service Development                         |
// +----------------------------------------------------------------------+
...
*/';
   return 'v.'||c_engversion||'.0.20160226225530'; 
 ...
 --   v_text     rasd_links.text%type;
  ...
  /*  select pov.type, pov.location, pov.text, upper(f.form)
    ...
    if v_type = 'S' then
      ...
      v_url := 'javascript: var link=window.open('''''||v_url||''''','''''||c_openLOVname||''''',''''resizable,scrollbars,width=680,height=550'''');';
    end if;
    return(v_url);
  end;
        ...
        if r.namecid is not null then
          begin
            select distinct r.namecid || decode(p.blockid, null, '', '_1')
             ...
             where upper(f.form) = upper(v_text)
    ...
    for rpov in (select distinct gpov.text lsql, gpov.linkid, gp.element
      ...
      if ptype = 'G' then
        ...
        if upper(rpov.element) = 'SELECT_' then
          ...
          addcnl('--</lovsql>');
          addcnl('    ) loop');
          addcnl('      htp.p(''  document.write(''''<option class=rasdSelectp ''''+ ((pvalue==''''''||r__.id||'''''')?''''selected'''':'''''''') +'''' value="''||r__.id||''">''||replace(r__.label,'''''''',''\'''''')||'' '''')'');');
          addcnl('    end loop;');
          addcnl('    htp.p(''}'');');
...
--                     and nvl(po.element,'INPUT_TEXT') not in ('FONT_RADIO', 'SELECT_')
      ...
      /*  for r in (select '  elsif upper(name_array(i)) = upper(''' ||
        ...
        begin
        addcnl('    htp.prn(''');
        rasd_enginehtml10x.writeHTMLHead(pformid);
        addcnl('    '');');
       ...
       /* for r in (select l.fieldid,
        ...
        addcnl(' htp.p(''Filter:<input type="text" name="PID" value="''||p_id||''" ></BR><input type="submit" class="rasdButton" value="Search"><input class="rasdButton" type="button" value="Clear" onclick="document.''||p_lov||''.PID.value=''''''''; document.''||p_lov||''.submit();"></BR>'');');        
    ...
    for rf in c_form(p_formid) loop
      ...
      for rpro in (select plsql
                    ...
                    where formid = rf.formid
                      and blockid is null
                      and upper(triggerid) not in
                       (select tctype from RASD_TRIGGERS_CODE_TYPES t where (language = 'P' and tclevel = 'F' or language = 'J' and tclevel = 'F' or language = 'C' and tclevel = 'F'))
      ...
      addcnl('       begin');     
      addcnl('        return  ''<script type="text/javascript">');
    for r in (select p.plsql, p.plsqlspec
                from rasd_triggers p
               where p.formid = rf.formid
                 and (blockid is null)
                 and upper(triggerid) = 'FORM_JS') loop
        addcnl(replace(r.plsql,'''',''''''));
    end loop; 
      addcnl('        </script>'';');
      addcnl('       end; ');
      ...
      addcnl('       begin');
      addcnl('        return ''<style type="text/css">');
    for r in (select p.plsql, p.plsqlspec
                from rasd_triggers p
               where p.formid = rf.formid
                 and (blockid is null)
                 and upper(triggerid) = 'FORM_CSS') loop
        addcnl(replace(r.plsql,'''',''''''));
    end loop; 
      addcnl('        </style>'';');
      addcnl('       end; ');
      ...
      for rp in c_fieldsOfForm(rf.formid) loop  
        if rp.element not in ('FONT_' , 'A_' , 'IMG_' ,'PLSQL_', 'INPUT_BUTTON', 'INPUT_SUBMIT', 'INPUT_RESET')
...
--        end if;
      ...
      addcnl('    htp.prn(''');
      rasd_engineHTML10x.writeHTML(rf.formid);
      addcnl('    '');');
      /*tt := '  exception' || c_nl;
...
--              getDefaultValue(rf.formid, null, 'GBUTTONSAVE') || ' then' || c_nl;
        ...
        if c_debug then
           ...
           addcnl('    htpClob(''<div class="debug">''||log__||''</div>'');');
                   

      addcnl('exception');
      addcnl('  when ' || re.library || '.e_finished then null;');
      addcnl('  when others then');
    
      -- HTML error handler
      addcnl('    htp.prn(''');
      rasd_engineHTML10x.writeHTMLError(rf.formid);
      addcnl('    '');');
      --
      /*        
        tt := 'exception' || c_nl;
