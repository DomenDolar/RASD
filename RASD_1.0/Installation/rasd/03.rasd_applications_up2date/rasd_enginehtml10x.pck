create or replace package rasd_engineHTML10x is
/*
// +----------------------------------------------------------------------+
...
create or replace package body rasd_engineHTML10x is
/*
// +----------------------------------------------------------------------+
// | RASD - Rapid Application Service Development                         |
// +----------------------------------------------------------------------+
...
*/';
   return 'v.'||rasd_engine10x.c_engversion||'.0.20151202225530'; 
 ...
 function getNextElementid(p_formid rasd_elements.formid%type)
  ...
  begin
    if p_out = 'Y' then
      if p_type = 'A' and p_value is not null then
        v_text := nvl(p_text, ' ') || p_name || '="' || p_value || '"';
      elsif p_type = 'A' and p_value is null then
      ...
      if p_type = 'A' then
        rasd_engine10x.addc(nvl(p_textcode, ' ') || p_forloop || p_name ||
                       p_valuecode || p_endloop);
      else
        rasd_engine10x.addc(p_textcode || p_forloop || p_name ||
                       p_valuecode || p_endloop);
      end if;
...
--          
        ...
        values
          (rb.formid,
           ...
           upper(rb.blockid) || '_BLOCK',
           0);
        --label block, v_elementid = elementid
        begin
          select elementid
            into v_elementid
            from rasd_elements
    ...
    --fields
      ...
      if n = 0 then
        ...
        if rp.blockid is null then
          ...
          if rp.element in ('INPUT_SUBMIT' , 'INPUT_RESET' , 'INPUT_BUTTON') then
          ...
          begin
            ...
            if re.element = 'TR_' then
              ...
              if rp.element = 'INPUT_HIDDEN' then
  ...
  begin
    ...
    loop
      ...
      if substr(p_text, i + 2, 1) = '=' then
        ...
        v_plsql := replace(rasd_engine10x.transformPLSQL(p_formid, p_blockid,v_plsql),'(i__)','(i'||p_blockid||')');
...
select a.rowid rid, p.nameid, 
     decode(a.type,'S',''');  if  ShowField'||p.nameid||'  then  htp.prn(''')||a.name name,
     a.valuecode||decode(a.type,'S',decode(a.valuecode,'/>',''');  end if;  htp.prn(''',''), ''');  end if;  htp.prn(''')  valuecode
...
where n.formid = p_formid and n.element in ('FORM_HEAD','FORM_FOOTER')
                ...
                from (select e.formid,
                      ...
                      select e.formid,
                             ...
                             decode(nvl(b.numrows, 1),
                                    ...
                                    p.fieldid || '.count loop htp.prn(''') forloop,
                             decode(nvl(b.numrows, 1),
                                    1,
                                    decode(nvl(b.emptyrows, 0),
                                           0,
                                           '',
                                           '''); end loop; htp.prn('''),
                                    '''); end loop; htp.prn(''') endloop
                        from (select formid,
                                     blockid,
                                     min(blockid || fieldid) keep(dense_rank first order by length(blockid || fieldid), blockid || fieldid) fieldid
                                from rasd_fields
  ...
  begin
    ...
    rasd_engine10x.addc('<html>');
    outputElement(p_formid, re.elementid, p_out, re.elementid);
    rasd_engine10x.addc('<body>');
    
    re          := getElement(p_formid, 'FORM_LAB');
    outputElement(p_formid, re.elementid, p_out, re.elementid);
    
    rasd_engine10x.addc('<div class="rasdForm">');
    
    rasd_engine10x.addc('<div class="rasdFormHead"><input onclick="history.go(-1);" type="button" value="'|| 'Back' ||'" class="rasdButton"></div>');
    
    rasd_engine10x.addc('<div class="rasdHtmlError">');
    rasd_engine10x.addc('  <div class="rasdHtmlErrorCode">''||sqlcode||''</div>');
    rasd_engine10x.addc('  <div class="rasdHtmlErrorText">''||sqlerrm||''</div>');   
    rasd_engine10x.addc('</div>');

    rasd_engine10x.addc('<div class="rasdFormFooter"><input onclick="history.go(-1);" type="button" value="'|| 'Back' ||'" class="rasdButton"></div>');

    rasd_engine10x.addc('</div>');

    rasd_engine10x.addc('</body><html>');
  end;
