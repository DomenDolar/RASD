create or replace package rasd_engine11 is
/*
// +----------------------------------------------------------------------+
...
// +----------------------------------------------------------------------+
*/
  c_engserver  rasd_engines.server%type := 'rasd_engine11';
  c_engclient  rasd_engines.client%type := 'rasd_engineHTML11';
  ...
  c_warning    rasd_fields.fieldid%type := 'WARNING';
  c_error    rasd_fields.fieldid%type := 'ERROR';
...
';
  ...
  function trueTrigger(pformid    number,
                       ...
                       pblockid   varchar2 default null) return boolean;
  function createTriggerTemplatePLSQL(pformid    number,
                               ptriggerid rasd_triggers.triggerid%type,
                               pblockid   varchar2 default null,
                               pdefault   varchar2 DEFAULT null) return clob;                       
...
20200130 Modifications on metadata procedure
20200128 Added Error trace log   
20200127 Changed pLOVType output from 300chars to 500chars
20200108 Changed metadata_xml presentation - form part is allways created   
20191210 Uncommented jsTRUEFALSE(P)   
20191210 Changed SQL LOV execution. If FETCH FIRST it is added to the end of SQL (line 2480)
20191210 Changed DEBUG log for fields type L-CLOB (only lengtht of L field type is displayed)
20191129 Commented part for generating CUSTOM PROCEDURES, FUNCTIONS, TYPES (line 3111)   
20191018 Ticket #28 solved. Added chnage trigger on close LOV
20191018 Extension of rlog function (added remote logging callLog in RASD_CLIENT).
20191018 Added function openlov( lov_name like lov$+++ , filter). Function returns table of records (id, label).
20190628 Added new condition to generatePLSQL - c_nl   
...
20190401 Change on on_new_record. Now you can put only changes into trigger not whole code for inicialization. Trigger on_new_record is now executed on non SQL blocks to.  
20190313 Some changes in generatePLSQL code   
20190221 Added VS - VisualSettings for error, readonly and custom    
20190201 Added in Links parameter #THIS_FORM#. Now you can use this when callin the same form instead hardcoded form name.
20190201 Added functionality in OpenLov. If you would like to return label from LOV then just create field with the same name where LOV is connected and add postfix LABEL. Added autofocus in filter field.
20181221 Added _rasd_link_url for DYNAMCFIELDS      
20181207 Added posted hidden fields and session fields to openLOV (limit 32k).   
20181129 Added split function if conntent in trigger is more than 32k (32000/2) in GEN 11.   
20181022 Added custom DYNAMICFIELD elements to engine   
20180807 Added #SET to FORM fields 
20180607 Added trigger tamplates where you can specify code for all generated programs.   
...
*/';
  ...
  procedure generatePLSQL is
    ...
    lxc  pls_integer;
    lx   pls_integer;
    vstrtmp varchar2(256);    
    lengthc pls_integer := 128; -- set to 128 because chars larger then 1byte. dbms_sql.varchar2s has limit 256byts
  begin
    z := 0;
    s.delete;
    for j in 1 .. v_gc.count loop
     begin  
      ...
      loop
        ...
        --Part added because of plsql code larger than 4000 chars in row when html is presented. So we split HTML in two rows where strng >< appears 13.3.2019
        if lx > 4000 then 
        lxc := lxc + length(s(z));
        if lxc > 3000 then 
           if instr(s(z), c_nl) > 0 then
             vstrtmp := s(z);
             s(z) := substr(vstrtmp,1, instr( vstrtmp, c_nl )-1 );            
             z := z + 1;
             s(z) := c_nl;
             z := z + 1;
             s(z) := substr(vstrtmp, instr( vstrtmp, c_nl )  );                         
             lxc := 0;
          elsif instr(s(z), '><') > 0 then
             vstrtmp := s(z);
             s(z) := substr(vstrtmp,1, instr( vstrtmp, '><' ) );            
             z := z + 1;
             s(z) := c_nl;
             z := z + 1;
             s(z) := substr(vstrtmp, instr( vstrtmp, '><' ) + 1 );                         
             lxc := 0;
          end if;   
        end if;  
        end if;
      ...
      exception when others then
        raise_application_error('-20000', 'Error generateing PLSQL(generatePLSQL j='||j||', i='||i||'): ' || SQLERRM||'Trace:'||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
      end;      
    end loop;
  ...
  begin
    ...
    while instr(v_plsql , '''') > 0 and ix < 10000000 loop
       ...
       if v_k > 0 then 
           if length(substr(v_plsql , v_z , v_k-v_z+1 )) > (32000/2) then

             v_kx := v_z + instr(substr(v_plsql , v_z , v_k-v_z+1 ),'
' ,(32000/2)
)-1;
             if v_kx = 0 then v_kx := v_k; end if;
           vsez(ix) := substr(v_plsql , v_z , v_kx-v_z+1 );
           v_plsql := replaceUpper(v_plsql, vsez(ix), '##RASD'||ix||'##'); 
                   
           ix := ix+1;
           vsez(ix) := substr(v_plsql , v_kx+1 , v_k-v_kx );
           v_plsql :=  replaceUpper(v_plsql, vsez(ix), '##RASD'||ix||'##');                       
  ...
  function createTriggerTemplatePLSQL(pformid    number,
                               ptriggerid rasd_triggers.triggerid%type,
                               pblockid   varchar2 default null,
                               pdefault   varchar2 DEFAULT null) return clob is
                               
    v_plsqlpre rasd_triggers_template.preplsql%type := '';
    v_plsqlon rasd_triggers_template.onplsql%type := pdefault;--||c_nl;
    v_plsqlpost rasd_triggers_template.postplsql%type := '';
  begin
    -- search for good trigger  
    for r in (
select t.preplsql , t.onplsql , t.postplsql
from rasd_triggers_template t, (select max(lobid) lobid from rasd_forms_compiled f where f.formid = pformid) f
where (t.lobid = f.lobid or t.lobid is null) 
  and upper(t.triggerid) = upper(ptriggerid)
order by decode(t.lobid,null,2,1)
      ) loop
      v_plsqlpre := r.preplsql;    
      if r.onplsql is not null then v_plsqlon  := r.onplsql ;end if;
      v_plsqlpost := r.postplsql;
      exit;
    end loop;
    return v_plsqlpre||v_plsqlon||v_plsqlpost;
  end;  
...
';
  ...
  begin
    ...
    for r in (select max(decode(b.pagingyn, c_true, 1, 0)) paging,
      ...
      select count(*)
       ...
       where p.formid = pformid
         and p.blockid is null
         and upper(p.fieldid) = upper(c_error);
      if n = 0 then
        insert into rasd_fields
          (formid,
           fieldid,
      ...
      select count(*)
       ...
       where p.formid = pformid
         and p.blockid is null
         and upper(p.fieldid) = upper(c_warning);
      if n = 0 then
        insert into rasd_fields
          (formid,
           fieldid,
 ...
 --   v_text     rasd_links.text%type;
  ...
  begin
                  ...
                  select min(upper(f.form)),  min(upper(p.element)) into v_form , v_element
                   ...
                   where p.formid = pformid
                     ...
                     and ( nvl(p.element,'INPUT_TEXT') in ('INPUT_TEXT')  or p.element like 'DYNAMICFIELD_%')
  ...
  /*  select pov.type, pov.location, pov.text, upper(f.form)
    ...
    if v_type = 'S' then
      ...
      if instr( v_element , 'DYNAMICFIELD') > 0 then

-- You can call in your DYNAMIC filed like ...
-- $.getJSON( link , { ''''restrestype'''' : ''''JSON'', ''''call'''': ''''REST'''' }).done(function( lov ){ lov_result=lov.openLOV.result; });
-- var xx = window.open( link ,''''resizable,scrollbars,width=680,height=550'''');

      v_url :=  ' var _rasd_url_type=''''LOV''''; var _rasd_url_loc=''''N''''; var _rasd_link_url=encodeURI(''''!' || v_form || '.' || c_openLOVname || '?PLOV=' || plinkid ||'&FIN='||v_form||'.''||name||''&PID=''''+document.' || v_form || '.''||name||''.value); ';
  ...
  function addLINK(pformid  rasd_links.formid%type,
    ...
    v_element  rasd_fields.element%type;
  begin
    select pov.type, pov.location, replace(pov.text,'#THIS_FORM#', f.form), f.form
      into v_type, v_location, v_text, v_form
      from rasd_links pov, rasd_forms f
     where pov.formid = pformid
       and pov.linkid = plinkid
      ...
      if ptype = 'G' then
        ...
        if upper(rpov.element) = 'SELECT_' then
          ...
          addcnl('    htp.p(''function js_S' || rpov.linkid ||
                 '(pvalue, pobjectname) {'');');
          addcnl('      htp.p('' eval(''''val_''''+pobjectname+''''="''''+pvalue+''''"'''');''); ');                 
...
--          addcnl('      htp.p(''  document.write(''''<option class=rasdSelectp ''''+ ((pvalue==''''''||r__.id||'''''')?''''selected'''':'''''''') +'''' value="''||r__.id||''">''||replace(r__.label,'''''''',''\'''''')||''</option>'''')'');');
          ...
          addcnl('      raise_application_error(''-20000'',''Error in '|| rpov.linkid ||': ''||sqlerrm);  ');
--          addcnl('      raise_application_error(''-20000'','''||RASDI_TRNSLT.text('Error in', p_lang)||' '|| rpov.linkid ||': ''||sqlerrm);  ');
          addcnl('    end;');
          addcnl('    htp.p(''}'');');
  ...
  --                 r.code ||
    --               ''''')?'''' checked '''':'''''''') +'''' value="' ||
      --             r.code || '">' || setHTP(r.value) || ''''')'');');
          ...
          addcnl('    htp.p(''function js_C' || rpov.linkid ||
                 'init(pvalue, pobjectname) {'');');          
          begin
          ...
          addcnl('    htp.p(''          var element = document.getElementById(pobjectname + ''''_RASD'''');  '');');
          addcnl('    htp.p(''          if (element.value == '''''||v_true||''''') {  element.checked = true;  }  '');');
          addcnl('    htp.p(''          else { element.checked = false;  } '');');

          addcnl('    htp.p(''}'');');  
          addcnl('    htp.p(''function js_C' || rpov.linkid ||
                 'click(pvalue, pobjectname) {'');');          
         
          addcnl('    htp.p(''          var element = document.getElementById(pobjectname + ''''_RASD'''');  '');');
          addcnl('    htp.p(''          if (element.checked) { element.value='''''||v_true||'''''; } else { element.value='''''||v_false||''''';}  '');');
  ...
  begin
    ...
    if n > 0 then
      if ptype = 'G' then
        ...
        addcnl(' procedure on_session;');
        
        addcnl('');
        addcnl('function ' || c_openLOVname || '(');
        addcnl('  p_lov varchar2,');
        addcnl('  p_value varchar2');
        addcnl(') return lovtab__ is');
        addcnl('  name_array   owa.vc_arr;');
        addcnl('  value_array  owa.vc_arr;');     
        addcnl('begin');
        addcnl('  name_array(1) := ''PLOV'';');
        addcnl('  value_array(1) := p_lov;');
        addcnl('  name_array(2) := ''PID'';');
        addcnl('  value_array(2) := p_value;');
        addcnl('  name_array(3) := ''CALL'';');
        addcnl('  value_array(3) := ''PLSQL'';  ');
        addcnl('  ' || c_openLOVname || '(name_array, value_array);');
        addcnl('  return lov__;');
        addcnl('end;');
...
--                     and nvl(po.element,'INPUT_TEXT') not in ('FONT_RADIO', 'SELECT_')
          ...
          addcnl('--<lovsql formid="' || pformid || '" linkid="' ||
                 r.linkid || '">');
          if instr (upper(r.text),'FETCH FIRST' , -1 ) = 0 then
            addcnl('select id, label from ('||r.text||' ) where upper(id) like ''%''||upper(p_id)||''%'' or upper(label) like ''%''||upper(p_id)||''%'' ');          
          else 
            declare
             vvv_strt number := instr (upper(r.text),'FETCH FIRST' , -1);
             vvv_fetch varchar2(4000) := substr( r.text , vvv_strt );
            begin
            
            if instr(vvv_fetch,')') = 0  and instr(lower(vvv_fetch),'select') = 0  and instr(lower(vvv_fetch),'else') = 0 and instr(lower(vvv_fetch),'from') = 0  then            
               addcnl('select id, label from ('||substr(r.text,1,vvv_strt-1)||' ) where upper(id) like ''%''||upper(p_id)||''%'' or upper(label) like ''%''||upper(p_id)||''%'' '||vvv_fetch);    
            else
               addcnl('select id, label from ('||r.text||' ) where upper(id) like ''%''||upper(p_id)||''%'' or upper(label) like ''%''||upper(p_id)||''%'' ');                        
            end if;   
            end;         
        ...
        addcnl('  v_hidden_fields varchar2(32000) := '''';');
        addcnl('  v_opener_tekst  varchar2(32000) := '''';');
      ...
      /*  for r in (select '  elsif upper(name_array(i)) = upper(''' ||
        ...
        addcnl('    else ');
        addcnl('      if name_array(i) not in (''LOVlist'') then');
        addcnl('        v_hidden_fields := v_hidden_fields||''<input type="hidden" name="''||name_array(i)||''" value="''||value_array(i)||''" />''; ');
        addcnl('      end if;');
        addcnl('    end if;');
        ...
        for r in (select distinct p.text, p.linkid, p.link
          ...
          if v_firstrecord then
            addcnl('  if lower(p_lov) = lower(''' || r.linkid || ''') then');
          else
            addcnl('  elsif lower(p_lov) = lower(''' || r.linkid || ''') then');
          end if;
          ...
          addcnl('    v_counter := v_counter - 1;');
          addcnl('        if 1=2 then null;');
          for r1 in (select nameid , decode(nvl(element,'INPUT_TEXT') ,'INPUT_TEXT' , 'value','TEXTAREA_','value','INPUT_HIDDEN','value', 'DYNAMICFIELD_INPUT', 'value','html') out
from rasd_fields f 
where f.formid = pformid
  and (f.fieldid, f.blockid) in (
select f.fieldid||'LABEL' , f.blockid 
from rasd_fields f 
where f.formid = pformid
  and f.linkid = r.linkid
  )) loop
          addcnl('        elsif     substr(p_nameid,instr(p_nameid,''.'')+1,instr(p_nameid,''_'',-1)-1-instr(p_nameid,''.''))||''LABEL'' = '''||r1.nameid||''' then ');
          if r1.out = 'value' then
          addcnl('          v_opener_tekst :=  ''window.opener.''||substr(p_nameid,1,instr(p_nameid,''_'',-1)-1)||''LABEL''||substr(p_nameid,instr(p_nameid,''_'',-1))||''.value = tekst'';          ');
          else 
          addcnl('          v_opener_tekst :=  ''window.opener.''||''$(''''#''||substr(p_nameid,instr(p_nameid,''.'')+1,instr(p_nameid,''_'',-1)-1-instr(p_nameid,''.''))||''LABEL''||substr(p_nameid,instr(p_nameid,''_'',-1))||''_RASD'''').html(tekst);'';          ');
          end if;
          end loop;
          addcnl('        end if;          ');
        ...
        for r in (select p.text, p.linkid, p.link
          ...
          for r1 in (select gp.value tekst,
                      ...
                      where gp.formid = pformid
                        ...
                        and gp.type = 'TEXT'
                      order by gp.orderby, value) loop
...
where f.formid = pformid
  and f.linkid = r.linkid
        ...
        addcnl('  lov__.delete;');
        addcnl('  for i in 1..v_lov.count loop');
        addcnl('   lov__(i).id := v_lov(i).p1;');
        addcnl('   lov__(i).label := v_lov(i).output;   ');
        addcnl('  end loop;  ');
  
        addcnl('elsif v_call = ''REST'' then ');
        ...
        begin
        addcnl('    htp.prn(''');
        rasd_engineHTML11.writeHTMLHead(p_formid => pformid,p_lang => p_lang);
        addcnl('    '');');
        exception when others then
        addcnl('    '');');
        end;
        ...
        addcnl('        $(function() {');
        addcnl('        document.getElementById("PID").select();');
        addcnl('        });');
        ...
        addcnl('     ''||v_opener_tekst||''');


        addcnl('     event = new Event(''''change'''');');
        addcnl('     window.opener.''||p_nameid||''.dispatchEvent(event);');
       ...
       /* for r in (select l.fieldid,
        ...
        addcnl(' htp.p(v_hidden_fields);');
        addcnl(' htp.p(''<div class="rasdLov" align="center"><center>'');');
        addcnl(' htp.p('''||RASDI_TRNSLT.text('Filter', p_lang)||':<input type="text" id="PID" autofocus="autofocus" name="PID" value="''||p_id||''" ></BR><input type="submit" class="rasdButton" value="'||RASDI_TRNSLT.text('Search', p_lang)||'"><input class="rasdButton" type="button" value="'||RASDI_TRNSLT.text('Clear', p_lang)||'" onclick="document.''||p_lov||''.PID.value=''''''''; document.''||p_lov||''.submit();"></BR>'');');        
        ...
        for r in (select distinct p.text, p.linkid, p.link
          ...
          for rp in (select p.nameid,
                      ...
                      where l.linkid = p.linkid
                        ...
                        and p.formid = pformid
                        and l.type in ('S','T')
                        and p.linkid = r.linkid
                      order by length(p.nameid) desc, p.nameid ) loop
            addcnl('    elsif name like ''' || rp.nameid || '%'' then');
            addcnl('      htp.prn(''' ||
...
if rf.autocreaterestyn = 'Y' then      
      ...
      addcnl('    required boolean default false,');    
      addcnl('    error varchar2(4000) ,');    
      addcnl('    info varchar2(4000) ,');    
      addcnl('    custom   varchar2(256)');    
      addcnl('  );');
      ...
      for rpf in c_FieldsOfForm(rf.formid) loop
        ...
        if nvl(rpf.INCLUDEVIS, c_false) = c_true then
        addcnl('  ' || rpad(rpf.fieldid||'#SET', 30, ' ') ||' set_type;');          
        end if;        
      end loop;
      ...
      for rpro in (select plsql
                    ...
                    where formid = rf.formid
                      ...
                      and upper(triggerid) not like '%REF(%'
                     -- and plsqlspec is null  --Commented 29.11.2019 - With this cooment lower code can be put out.
                     and plsql is not null
                      order by triggerid) loop
        addcnl(rpro.plsql);
      end loop;

/*  Commented 29.11.2019
      for rpro in (select plsql
                     from rasd_triggers
                    where formid = rf.formid
                      and blockid is null
...
if rf.autocreaterestyn = 'Y' then      
      ...
      addcnl('       begin');
      addcnl('        log__ := log__ ||systimestamp||'':''||v_clob||''<br/>'';');
      

      addcnl('        '||c_englibrary||'.callLog('''||rf.form ||''',v_clob, systimestamp, '''' );');
      ...
      addcnl('       begin');     
      addcnl('        return  ''');
    if trueTrigger(rf.formid,'FORM_UIHEAD','') then  
    for r in (select p.plsql, p.plsqlspec
                from rasd_triggers p
               where p.formid = rf.formid
                 and (blockid is null)
                 and  instr(triggerid, 'FORM_UIHEAD' ) > 0 and triggerid = 'FORM_UIHEAD'                
                 ) loop
        addcnl(setHTP(replace(r.plsql,'''','''''')));
    end loop; 
    else
      addcnl(setHTP(replace( createTriggerTemplatePLSQL(rf.formid, 'FORM_UIHEAD' , '','' ) ,'''',''''''))); 
    end if;    
    for r in (select p.plsql, p.plsqlspec
                from rasd_triggers p
               where p.formid = rf.formid
      ...
      addcnl('       begin');     
      addcnl('        return  ''');
    if trueTrigger(rf.formid , 'FORM_JS' ,'') then
    for r in (select p.plsql, p.plsqlspec  -- SET JS CODE
                from rasd_triggers p
               where p.formid = rf.formid
                 and (blockid is null)
--                 and upper(triggerid) in ('FORM_JS','FORM_JS_REF')
                 and  instr(triggerid, 'FORM_JS' ) > 0 and triggerid = 'FORM_JS'               
                 ) loop
        addcnl(setHTP(replace(r.plsql,'''','''''')));
    end loop; 
    else 
      addcnl( setHTP(replace(createTriggerTemplatePLSQL(rf.formid,'FORM_JS',''),'''','''''')) );      
    end if;
    for r in (select p.plsql, p.plsqlspec  -- SET REFERENCED CODE
                from rasd_triggers p
               where p.formid = rf.formid
                 and (blockid is null)
--                 and upper(triggerid) in ('FORM_JS','FORM_JS_REF')
    ...
    if trueTrigger(rf.formid , 'FORM_CSS' ,'') then
    for r in (select p.plsql, p.plsqlspec -- SET CSS CODE
                from rasd_triggers p
               where p.formid = rf.formid
                 and (blockid is null)
--                 and upper(triggerid) in('FORM_CSS','FORM_CSS_REF')
                 and  instr(triggerid, 'FORM_CSS' ) > 0 and triggerid = 'FORM_CSS'
                 ) loop
        addcnl(setHTP(replace(r.plsql,'''','''''')));
    end loop; 
    else
      addcnl( setHTP(replace(createTriggerTemplatePLSQL(rf.formid,'FORM_CSS',''),'''','''''')) );        
    end if; 
    for r in (select p.plsql, p.plsqlspec -- SET REFERENCED CODE
                from rasd_triggers p
               where p.formid = rf.formid
...
--                 and upper(triggerid) in('FORM_CSS','FORM_CSS_REF')
      ...
      for rp in c_fieldsOfForm(rf.formid) loop  
        ...
        if nvl(rp.element,'INPUT_TEXT') not in ('FONT_' , 'A_' , 'IMG_' ,'PLSQL_', 'INPUT_BUTTON', 'INPUT_SUBMIT', 'INPUT_RESET')
           ...
           and rp.fieldid <> c_error
           and rp.fieldid <> c_warning
...
if rb.dbblockyn = c_true then
      ...
      addcnl('    for i__ in 1..nvl(num_entries,0) loop');
        ...
        if c_debug then
            ...
            addcnl('rlog(''''||name_array(i__)||'' *LIMIT to show in DEBUG 100 chars* =''||substr(value_array(i__),1,100)||'';'');');
        end if;
      ...
      for rpf in c_fieldsOfForm(rf.formid) loop
        ...
        if c_debug then
           if  rpf.type = 'L' then
            addc('rlog('''|| rpf.fieldi||' LENGTH=''||length('||rpf.fieldi||')||'';'');');
           else  
      ...
      for rpb in c_fieldsOfBlock(rf.formid) loop
        ...
        if c_debug then
           if rpb.type = 'L' then
            addc('rlog('''|| rpb.blockid || rpb.fieldid ||'(''||to_number(substr(name_array(i__),instr(name_array(i__),''_'',-1)+1))||'') LENGTH= ''||length('||rpb.blockid || rpb.fieldid||'(to_number(substr(name_array(i__),instr(name_array(i__),''_'',-1)+1))))||'';'');');                       
            else
      ...
      for rpb in c_fieldsOfBlock(rf.formid) loop
        ...
        if c_debug then
           if rpb.type = 'L' then
            addc('rlog('''|| rpb.blockid || rpb.fieldid ||'(1) LENGTH= ''||length('||rpb.blockid || rpb.fieldid||'(1))||'';'');');
           else
      ...
      for rb in (select blockid, numrows  from rasd_blocks where formid = rf.formid) loop
        ...
        for rpb in c_fieldsOfBlock(rf.formid, rb.blockid) loop
          ...
          if nvl(rpb.includevis,c_false) = c_true then
            addcnl('      if not ' || rpb.blockid || rpb.fieldid ||'#SET'||
                ...
                if rpb.notnullyn = c_true then
                addcnl('        ' || rpb.blockid || rpb.fieldid || '#SET(i__).required := true;');  
                end if;                        
            addcnl('      end if;');
          end if;
        ...
        if trueTrigger(rf.formid, 'on_new_record', rb.blockid) then
          createTriggerPLSQL(rf.formid, 'on_new_record', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'on_new_record' , '' ,''));
        end if;
      ...
      if trueTrigger(rf.formid, 'POST_SUBMIT') then
        createTriggerPLSQL(rf.formid, 'POST_SUBMIT', '' , createTriggerTemplatePLSQL(rf.formid, 'POST_SUBMIT', '', '-- Executing after filling fields on submit.'));
      else
        addcnl(createTriggerTemplatePLSQL(rf.formid, 'POST_SUBMIT', '', ''));        
      end if;
      addcnl('    null;');
      addcnl('  end;');
      ...
      if trueTrigger(rf.formid, 'ON_SUBMIT') then
        createTriggerPLSQL(rf.formid, 'ON_SUBMIT' , '' , createTriggerTemplatePLSQL(rf.formid, 'ON_SUBMIT' , '' ,'-- Reading post variables into fields.'||c_nl||'    on_submit(name_array ,value_array); on_session;'));
      else
        addcnl(createTriggerTemplatePLSQL(rf.formid, 'ON_SUBMIT' , '' ,'-- Reading post variables into fields.'||c_nl||'    on_submit(name_array ,value_array); on_session;'));
      end if;
      addcnl('    post_submit;');
...
--        addcnl(  rb.blockid||c_rid );
        ...
        addcnl('      end;');          
         ...
         for rp in c_fieldsOfBlock(rf.formid, rb.blockid) loop
          if nvl(rp.INCLUDEVIS,c_false) = c_true then 
        addcnl('      begin');
        addcnl('      '|| rb.blockid || rp.fieldid||'#SET(i__) := '|| rb.blockid || rp.fieldid||'#SET(i__);');
        addcnl('      exception when others then');             
         addcnl('        ' || rb.blockid || rp.fieldid || '#SET(i__).visible := true;' || c_nl);
                if rp.notnullyn = c_true then
                addcnl('        ' || rp.blockid || rp.fieldid || '#SET(i__).required := true;');  
                end if;         
        addcnl('      end;');          
          end if;
        end loop;           
        ...
        for rp in c_fieldsOfBlock(rf.formid, rb.blockid) loop
          if nvl(rp.INCLUDEVIS,c_false) = c_true then
            tt := tt || '        ' || rb.blockid || rp.fieldid ||
                  '#SET(i__).visible := true;' || c_nl;
          
                if rp.notnullyn = c_true then
            tt := tt || '        ' || rb.blockid || rp.fieldid ||
                  '#SET(i__).required := true;' || c_nl;
                end if;          
...
--       change because on_new_record should be always full created 
/*        if trueTrigger(rf.formid, 'on_new_record', rb.blockid) then
          createTriggerPLSQL(rf.formid, 'on_new_record', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'on_new_record' , '' ,tt));
        else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'on_new_record' , '' ,tt));
        end if;*/

        addcnl(tt);       
        if trueTrigger(rf.formid, 'on_new_record', rb.blockid) then
          createTriggerPLSQL(rf.formid, 'on_new_record', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'on_new_record' , '' ,'-- Triggers after setting default values. Use (i__) to access fields values.'));
     --   else
      --    addcnl(createTriggerTemplatePLSQL(rf.formid, 'on_new_record' , '' ,tt));
        end if;
--        
...
--        end if;
      ...
      if trueTrigger(rf.formid, 'on_clear') then
        createTriggerPLSQL(rf.formid, 'on_clear', '', createTriggerTemplatePLSQL(rf.formid, 'on_clear' , '' ,tt));
      else 
        addcnl(createTriggerTemplatePLSQL(rf.formid, 'on_clear' , '' ,tt));         
      end if;
      ...
      for rb in c_blocks(rf.formid) loop
        ...
        if nvl(rb.dbblockyn, 'N') = c_true then
          for rp in c_fieldsOfBlock(rf.formid, rb.blockid) loop
            ...
            if trueTrigger(rf.formid, 'pre_select', rb.blockid) then
          createTriggerPLSQL(rf.formid, 'pre_select', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'pre_select' , rb.blockid ,'-- Triggers before executing SQL.'));
            else  
               addcnl(createTriggerTemplatePLSQL(rf.formid, 'pre_select' , rb.blockid ,''));
            end if;
          ...
          if trueTrigger(rf.formid, 'on_select', rb.blockid) then
            createTriggerPLSQL(rf.formid,
                               'on_select',
                               rb.blockid,
                              createTriggerTemplatePLSQL(rf.formid, 'on_select' , rb.blockid ,tt)
                              );
          else
            addcnl(createTriggerTemplatePLSQL(rf.formid, 'on_select' , rb.blockid ,tt));
          end if;
          addcnl('        i__ := 1;');
          addcnl('        LOOP ');
          ...
          */
          ...
          --record status
          if trueUpdate(rf.formid, rb.blockid) or trueDelete(rf.formid, rb.blockid)  then
          ...
          if trueTrigger(rf.formid, 'on_lock_value', rb.blockid) then
            createTriggerPLSQL(rf.formid, 'on_lock_value', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'on_lock_value', rb.blockid, tt));
          else
            addcnl(createTriggerTemplatePLSQL(rf.formid, 'on_lock_value', rb.blockid, tt));
          end if;
          --locking end
          if trueTrigger(rf.formid, 'post_select', rb.blockid) then
          createTriggerPLSQL(rf.formid, 'post_select', rb.blockid,createTriggerTemplatePLSQL(rf.formid, 'post_select', rb.blockid,'-- Triggers after executing SQL. Use (i__) to access fields values.'));
          else
            addcnl(createTriggerTemplatePLSQL(rf.formid, 'post_select', rb.blockid,''));
          end if;           
          --if nvl(rb.pagingyn, 'N') = c_true then
      ...
      for rb in c_blocks(rf.formid) loop
        ...
        if trueTrigger(rf.formid, 'on_validate', rb.blockid) then
        createTriggerPLSQL(rf.formid, 'on_validate', rb.blockid ,createTriggerTemplatePLSQL(rf.formid, 'on_validate', rb.blockid ,'-- Validating field values before DML. Use ('||v_i_record||') to access fields values.'));
        else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'on_validate', rb.blockid ,'-- Validating field values before DML. Use ('||v_i_record||') to access fields values.'));
        end if;
        ...
        if rb.dbblockyn = c_true then
          ...
          if trueInsertNN(rf.formid, rb.blockid) then
            ...
            if trueTrigger(rf.formid, 'pre_insert', rb.blockid) then
                createTriggerPLSQL(rf.formid, 'pre_insert', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'pre_insert', rb.blockid, '-- Triggers before INSERT. Use (i__) to access fields values.'));
            else  
               addcnl(createTriggerTemplatePLSQL(rf.formid, 'pre_insert', rb.blockid, ''));
            end if;
            ...
            if trueTrigger(rf.formid, 'on_mandatory', rb.blockid) then
               createTriggerPLSQL(rf.formid, 'on_mandatory', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'on_mandatory', rb.blockid, tt));
            else  
               addcnl(createTriggerTemplatePLSQL(rf.formid, 'on_mandatory', rb.blockid, tt));
            end if;
            ...
            if trueTrigger(rf.formid, 'on_insert', rb.blockid) then
              createTriggerPLSQL(rf.formid, 'on_insert', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'on_insert', rb.blockid, tt));
            else
              if trueInsert(rf.formid, rb.blockid) then
                addcnl(createTriggerTemplatePLSQL(rf.formid, 'on_insert', rb.blockid, tt));
              end if;
            end if;
                     
        if trueTrigger(RF.formid, 'post_insert', rb.blockid) then
            createTriggerPLSQL(RF.formid, 'post_insert', rb.blockid, createTriggerTemplatePLSQL(RF.formid, 'post_insert', rb.blockid, '-- Triggers after INSERT. Use (i__) to access fields values.'));
        else
          addcnl(createTriggerTemplatePLSQL(RF.formid, 'post_insert', rb.blockid, ''));
        end if;
          ...
          tt := tt||' if substr('||rb.blockid || c_rs||'(i__),1,1) = ''U'' then'||c_nl;
          tt := tt||' declare'||c_nl;
          ...
          if trueTrigger(rf.formid, 'on_lock', rb.blockid) then
            createTriggerPLSQL(rf.formid, 'on_lock', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'on_lock', rb.blockid, tt));
          else
            addcnl(createTriggerTemplatePLSQL(rf.formid, 'on_lock', rb.blockid, tt));
          end if;
          --end locking
          tt := '';
        ...
        if trueTrigger(rf.formid, 'pre_delete', rb.blockid) then
            createTriggerPLSQL(rf.formid, 'pre_delete', rb.blockid,createTriggerTemplatePLSQL(rf.formid, 'pre_delete', rb.blockid,'-- Triggers before DELETE. Use (i__) to access fields values.'));
        else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'pre_delete', rb.blockid,''));
        end if;
            ...
            if trueTrigger(rf.formid, 'on_delete', rb.blockid) then
              createTriggerPLSQL(rf.formid, 'on_delete', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'on_delete', rb.blockid, tt));
            else
              if trueDelete(rf.formid, rb.blockid) then
                addcnl(createTriggerTemplatePLSQL(rf.formid, 'on_delete', rb.blockid, tt));
              end if; -- Delete exists
            end if;

        if trueTrigger(rf.formid, 'post_delete', rb.blockid) then
            createTriggerPLSQL(rf.formid, 'post_delete', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'post_delete', rb.blockid, '-- Triggers after DELETE. Use (i__) to access fields values.'));
        else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'post_delete', rb.blockid, ''));
        end if;
        ...
        if trueTrigger(rf.formid, 'pre_update', rb.blockid) then
            createTriggerPLSQL(rf.formid, 'pre_update', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'pre_update', rb.blockid, '-- Triggers before UPDATE. Use (i__) to access fields values.'));
        else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'pre_update', rb.blockid, ''));
        end if;
          ...
          if trueTrigger(rf.formid, 'on_update', rb.blockid) then
            createTriggerPLSQL(rf.formid, 'on_update', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'on_update', rb.blockid, tt));
          else
            if trueUpdate(rf.formid, rb.blockid) then
              addcnl(createTriggerTemplatePLSQL(rf.formid, 'on_update', rb.blockid, tt));
            end if;
          end if;
          if trueUpdate(rf.formid, rb.blockid) then

        if trueTrigger(rf.formid, 'post_update', rb.blockid) then
            createTriggerPLSQL(rf.formid, 'post_update', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'post_update', rb.blockid, '-- Triggers after UPDATE. Use (i__) to access fields values.'));
        else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'post_update', rb.blockid, ''));
        end if;
      ...
      addcnl('  begin');
            ...
            if trueTrigger(rf.formid, 'pre_commit','') then
      createTriggerPLSQL(rf.formid, 'pre_commit','',createTriggerTemplatePLSQL(rf.formid, 'pre_commit','','-- Triggers before executing DML operations on DB blocks.'));
            else  
               addcnl(createTriggerTemplatePLSQL(rf.formid, 'pre_commit','',''));
            end if;
      ...
      if trueTrigger(rf.formid, 'post_commit','') then
      createTriggerPLSQL(rf.formid, 'post_commit','',createTriggerTemplatePLSQL(rf.formid, 'post_commit','','-- Triggers after executing DML operations on DB blocks.'));
      else
        addcnl(createTriggerTemplatePLSQL(rf.formid, 'post_commit','',''));
      end if; 
      addcnl('  null; ');
      ...
      jsSQL(rf.formid, 'P');
      jsTEXT(rf.formid, 'P');
      jsTRUEFALSE(rf.formid, 'P'); --uncommented 10.12.2019
      plsqlSQLTEXT(rf.formid, 'P', p_lang);
      
      rasd_engineHTML11.writePrepareHTML(p_formid => rf.formid,p_lang => p_lang);
      
      
      for rb in (select * from rasd_blocks where  formid =  rf.formid) loop
      
      if trueTrigger(rf.formid , 'pre_ui' , rb.blockid) then
      addcnl('procedure pre_output_'||rb.blockid||'_DIV is  begin');
   
      --      if trueTrigger(rf.formid, 'pre_ui', rb.blockid) then
          createTriggerPLSQL(rf.formid, 'pre_ui', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'pre_ui', rb.blockid, '-- Triggers before showing BLOCK content.'));
--            else  
  --             addcnl(createTriggerTemplatePLSQL(rf.formid, 'pre_ui', rb.blockid, ''));
    --        end if;
   
      addcnl('  end;');     
      end if;
      
      addcnl('procedure output_'||rb.blockid||'_DIV is begin htp.p(''');
      rasd_engineHTML11.outputelement( p_formid,  null,    'N',   rasd_engineHTML11.getElementBlock(p_formid , 'BLOCK_DIV' , ''||rb.blockid||'_DIV')   , null);
      addc('''); end;');    

      if trueTrigger(rf.formid , 'post_ui' , rb.blockid) then
      addcnl('procedure post_output_'||rb.blockid||'_DIV is  begin');

        -- if trueTrigger(rf.formid, 'post_ui', rb.blockid) then
          createTriggerPLSQL(rf.formid, 'post_ui', rb.blockid, createTriggerTemplatePLSQL(rf.formid, 'post_ui', rb.blockid, '-- Triggers before afther BLOCK content.'));
          --  else  
           --    addcnl(createTriggerTemplatePLSQL(rf.formid, 'post_ui', rb.blockid, ''));
          --  end if;
      ...
      addcnl('    htp.prn(''');
      rasd_engineHTML11.writeOutputHTML(p_formid => rf.formid,p_lang => p_lang);
      addcnl('    '');');
      /*tt := '  exception' || c_nl;
...
--      addcnl('  rasd_client.secCheckCredentials(  name_array , value_array ); ');
        ...
        if trueTrigger(rf.formid, 'PRE_ACTION') then
          createTriggerPLSQL(rf.formid, 'PRE_ACTION', '', createTriggerTemplatePLSQL(rf.formid, 'PRE_ACTION', '', tt));
        else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'PRE_ACTION', '', tt));
        end if;          
--      
...
--              getDefaultValue(rf.formid, null, 'GBUTTONSAVE') || ' then' || c_nl;
        ...
        if trueTrigger(rf.formid, 'ON_ACTION') then
          createTriggerPLSQL(rf.formid, 'ON_ACTION', '', createTriggerTemplatePLSQL(rf.formid, 'ON_ACTION', '', tt));
        else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'ON_ACTION', '', tt));
        end if;
        ...
        if trueTrigger(rf.formid, 'POST_ACTION') then
          createTriggerPLSQL(rf.formid, 'POST_ACTION', '', createTriggerTemplatePLSQL(rf.formid, 'POST_ACTION', '', tt));
        else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'POST_ACTION', '', tt));
        end if;
     ...
     begin
      ...
      rasd_engineHTML11.writeHTMLError(p_formid => rf.formid,p_lang => p_lang);      
      ...
      tt := tt  || v_gc(i);
        ...
        if trueTrigger(rf.formid, 'ON_ERROR') then
          v_gl := i_start;
          createTriggerPLSQL(rf.formid, 'ON_ERROR', '', createTriggerTemplatePLSQL(rf.formid, 'ON_ERROR', '', tt));
        else
          v_gl := i_start;
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'ON_ERROR', '', tt));  
        end if;
...
--              getDefaultValue(rf.formid, null, 'GBUTTONSAVE') || ' then' || c_nl;
        ...
        if trueTrigger(rf.formid, 'ON_ACTION_MAIN') then
          createTriggerPLSQL(rf.formid, 'ON_ACTION_MAIN', '', createTriggerTemplatePLSQL(rf.formid, 'ON_ACTION_MAIN', '', tt)); 
        else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'ON_ACTION_MAIN', '', tt));
        end if;
      ...
      if trueTrigger(rf.formid, 'ON_ERROR_MAIN') then
          createTriggerPLSQL(rf.formid, 'ON_ERROR_MAIN', '', createTriggerTemplatePLSQL(rf.formid, 'ON_ERROR_MAIN', '', tt));
      else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'ON_ERROR_MAIN', '', tt));
      end if;
      addcnl('end; ');
end if;
      /***************************************************************************************/
...
--              getDefaultValue(rf.formid, null, 'GBUTTONSAVE') || ' then' || c_nl;
        ...
        if trueTrigger(rf.formid, 'ON_ACTION_REST') then
          createTriggerPLSQL(rf.formid, 'ON_ACTION_REST', '', createTriggerTemplatePLSQL(rf.formid, 'ON_ACTION_REST', '', tt));
        else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'ON_ACTION_REST', '', tt));
        end if;
      ...
      if trueTrigger(rf.formid, 'ON_ERROR_REST') then
          createTriggerPLSQL(rf.formid, 'ON_ERROR_REST', '', createTriggerTemplatePLSQL(rf.formid, 'ON_ERROR_REST', '', tt));
      else
          addcnl(createTriggerTemplatePLSQL(rf.formid, 'ON_ERROR_REST', '', tt));
      end if;
      addcnl('end; ');
end if;
      /***************************************************************************************/
...
--if v_metadata > 0 then   conndition added to SQL
--addcnl('  /** '); 
--end if;
...
begin
select xmlelement( 
           "form", xmlelement("formid",f.formid) ,
            ...
            xmlelement("referenceyn",f.referenceyn) , 
            xmlelement("autodeletehtmlyn",f.autodeletehtmlyn) , 
            xmlelement("autocreaterestyn",f.autocreaterestyn) , 
            xmlelement("autocreatebatchyn",f.autocreatebatchyn) , 
            xmlelement("addmetadatainfoyn",(select decode(count(*),0,c_false,c_true) from rasd_forms_compiled where compileyn = c_true and formid = f.formid)) ,             
            xmlelement("compiler",
...
--            xmlelement("text2id",f.text2id) ,
           -- compile info
             (select xmlelement("compiledInfo" , XMLAgg(xmlelement("info" ,
               xmlelement("engineid",x.engineid),
               xmlelement("change",to_char(x.change,'dd.mm.yyyy hh/mi/ss')),
               xmlelement("compileyn",x.compileyn),
               xmlelement("application",x.application),
               xmlelement("owner",x.owner),
               xmlelement("editor",x.editor)
            ))) 
            from rasd_forms_compiled x where f.formid = x.formid and f.lobid = x.lobid ), 
            --rasd_blocks
            (select xmlelement("blocks", XMLAgg(xmlelement("block",
              ...
              xmlelement("rlobid",b.rlobid) ,
              xmlelement("rform",b.rform) ,
              xmlelement("rblockid",b.rblockid) ,             
              --rasd_fields 
              ...
              xmlelement("rlobid",p.rlobid) ,
              xmlelement("rform",p.rform) ,
              xmlelement("rblockid",p.rblockid) ,             
              xmlelement("rfieldid",p.rfieldid) ,             
              xmlelement("includevis",p.includevis)
                ))) from rasd_fields p where formid = f.formid and p.blockid = b.blockid )              
              ))) from rasd_blocks b where b.formid = f.formid and v_metadata > 0
              ) ,
            ...
            (select xmlelement("fields", XMLAgg(xmlelement("field",
              ...
              xmlelement("rfieldid",p.rfieldid) ,             
              xmlelement("includevis",p.includevis)
                ))) from rasd_fields p where formid = f.formid and p.blockid is null and v_metadata > 0 ) ,
             --rasd_links    
            (select xmlelement("links", XMLAgg(xmlelement("link",
             ...
             (select xmlelement("params" , XMLAgg(xmlelement("param" ,
              ...
              xmlelement("rlobid",c.rlobid) ,
              xmlelement("rform",c.rform) ,
              xmlelement("rlinkid",c.rlinkid)                
            ))) 
            from rasd_link_params c where b.linkid = c.linkid and b.formid = c.formid )
           ))) from rasd_links b where b.formid = f.formid and v_metadata > 0) ,
           --rasd_pages
            (select xmlelement("pages", XMLAgg(xmlelement("pagedata",
              ...
              xmlelement("rlobid",g.rlobid) ,
              xmlelement("rform",g.rform) ,
              xmlelement("rblockid",g.rblockid) ,             
              xmlelement("rfieldid",g.rfieldid)               
           ))) from rasd_pages g where g.formid = f.formid and v_metadata > 0),                           
           --rasd_triggers
...
')),
              ...
              xmlelement("rlobid",h.rlobid) ,
              xmlelement("rform",h.rform) ,
              xmlelement("rblockid",h.rblockid)               
           ))) from rasd_triggers h where h.formid = f.formid and v_metadata > 0) ,
             --rasd_elements    
            (select xmlelement("elements", XMLAgg(xmlelement("element",
              ...
              xmlelement("rlobid",k.rlobid) ,
              xmlelement("rform",k.rform) ,
              xmlelement("rid",k.rid) ,             
              xmlelement("includevis",k.includevis) ,              
              --rasd_attributes
             (select xmlelement("attributes" , XMLAgg(xmlelement("attribute" ,
              ...
              xmlelement("rlobid",l.rlobid) ,
              xmlelement("rform",l.rform) ,
              xmlelement("rid",l.rid)
            ))) 
            from rasd_attributes l where k.elementid = l.elementid and k.formid = l.formid )
           ))) from rasd_elements k where k.formid = f.formid and v_metadata > 0)                                       
