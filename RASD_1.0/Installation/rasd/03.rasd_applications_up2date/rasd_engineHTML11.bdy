...
';
  ...
  c_core  constant integer := 10; -- CORE FOR ELEMENTS, ATTRIBUTES
...
*/';
   return 'v.'||rasd_engine11.c_engversion||'.0.20180525225530'; 
...
select '''); if '||f.blockid||f.fieldid||'#SET(i'||f.blockid||').visible then htp.p('''||a.name svaluecode,
       ...
       '''); if '||f.blockid||f.fieldid||'#SET(i'||f.blockid||').required then htp.p('' required="required"''); end if; htp.p(''' qvaluecode,
         '<% if '||f.blockid||f.fieldid||'#SET(i'||f.blockid||').required then htp.p('' required="required"''); end if; %>' qvalue,       
       '''); htp.p('||f.blockid||f.fieldid||'#SET(i'||f.blockid||').custom); htp.p(''' tvaluecode,
         '<% htp.p('||f.blockid||f.fieldid||'#SET(i'||f.blockid||').custom); %>' tvalue,       
       a.* ,
       f.fieldid,
       f.blockid
...
values
  ...
  procedure writeHTML(p_formid rasd_elements.formid%type,
                      p_out    in varchar2 default 'N') is
  begin
    writePrepareHTML(p_formid, p_out); 
    writeOutputHTML(p_formid, p_out);
  end;
  ...
  begin
    ...
    rasd_engine11.addc('<div class="rasdFormHead"><input onclick="history.go(-1);" type="button" value="'|| 'Back' ||'" class="rasdButton"></div>');
    
    rasd_engine11.addc('<div class="rasdHtmlError">');
    rasd_engine11.addc('  <div class="rasdHtmlErrorCode">''||sqlcode||''</div>');
    rasd_engine11.addc('  <div class="rasdHtmlErrorText">''||sqlerrm||''</div>');   
    rasd_engine11.addc('</div>');

    rasd_engine11.addc('<div class="rasdFormFooter"><input onclick="history.go(-1);" type="button" value="'|| 'Back' ||'" class="rasdButton">');
