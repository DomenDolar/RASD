create or replace package IMP_DEMO_FIELDS_REF is
/*
// +----------------------------------------------------------------------+
// | RASD - Rapid Application Service Development                         |
//   Program: IMP_DEMO_FIELDS_REF generated on 30.01.20 by user RASDCLI.     
// +----------------------------------------------------------------------+
...
create or replace package body IMP_DEMO_FIELDS_REF is
/*
// +----------------------------------------------------------------------+
// | RASD - Rapid Application Service Development                         |
//   Program: IMP_DEMO_FIELDS_REF generated on 30.01.20 by user RASDCLI.    
  ...
  PXXXX                         dtab;
     ...
     procedure rlog(v_clob clob) is
       begin
        ...
        rasd_client.callLog('IMP_DEMO_FIELDS_REF',v_clob, systimestamp, '' );
       end; 
...
<html>');
    htp.prn('<head>
<head>');  htpClob(rasd_client.getHtmlJSLibrary('HEAD','DEMO_FIELDS REferenced FORM')); htpClob(FORM_UIHEAD); htp.p('<style type="text/css">'); htpClob(FORM_CSS); htp.p('</style><script type="text/javascript">'); htpClob(FORM_JS); htp.p('</script>');  
htp.prn('</head></head>
    ');
 htp.bodyOpen('','');
htp.p('
...
htp.p('this.close ();
 ...
 htp.formOpen(curl=>'!IMP_DEMO_FIELDS_REF.openLOV',
...
-- init fields
    ...
    for i__ in 1..v_max loop
      ...
      if not B10SAL#SET.exists(i__) then
        B10SAL#SET(i__).visible := true;
      end if;
      if not B10SALCALC.exists(i__) or B10SALCALC(i__) is null then
        B10SALCALC(i__) := 'L';
      end if;
      if not B10SALCALC#SET.exists(i__) then
        B10SALCALC#SET(i__).visible := true;
      end if;
...
.count(); end if;
      ...
      begin
      B10SAL#SET(i__) := B10SAL#SET(i__);
      exception when others then
        B10SAL#SET(i__).visible := true;

      end;
      begin
      B10SALCALC#SET(i__) := B10SALCALC#SET(i__);
      exception when others then
        B10SALCALC#SET(i__).visible := true;
...
-- Validating field values before DML. Use (i__) to access fields values.
  ...
  begin 
    if  nvl(PAGE,0) = 0 or nvl(PAGE,0) = 2 or nvl(PAGE,0) = 4 or nvl(PAGE,0) = 1 or nvl(PAGE,0) = 3 then 
       return true;
    end if;
    return false;
  end; 
  ...
  begin 
    if  nvl(PAGE,0) = 0 or nvl(PAGE,0) = 4 or nvl(PAGE,0) = 1 or nvl(PAGE,0) = 3 or nvl(PAGE,0) = 2 then 
       return true;
    end if;
    return false;
  end; 
  ...
  begin
    if 1=2 then null;
      ...
      v_return := v_return || 'document.IMP_DEMO_FIELDS_REF.ROWVALUE.value='''||replace(B10EMPNO(to_number(substr(name,instr(name,'_',-1)+1))),'"','&quot;')||
...
'''';
  ...
  begin
    if 1=2 then null;
      ...
      v_return := v_return || '''!demo_cform.webclient?B30MESSAGE_1=Send message from form 1&FIN=opener.js_LINK_CALL_DEMO_CFORM(\'''||name||'\'',this.document)&B10JOB_1=''+document.IMP_DEMO_FIELDS_REF.B10JOB2_'||substr(name,instr(name,'_',-1)+1)||'.value+
''&GBUTTONRES=''+document.IMP_DEMO_FIELDS_REF.B10JOB_'||substr(name,instr(name,'_',-1)+1)||'.value+
''&PFILTER_1='||replace(value,'"','&quot;')||'''';
    elsif name is null then
      v_return := v_return ||'''!demo_cform.webclient?B30MESSAGE_1=Send message from form 1&FIN=opener.js_LINK_CALL_DEMO_CFORM(\'''||name||'\'',this.document)&B10JOB_1=''+document.IMP_DEMO_FIELDS_REF.B10JOB2_1.value+
''&GBUTTONRES=''+document.IMP_DEMO_FIELDS_REF.B10JOB_1.value+
''&PFILTER_1='||replace(value,'"','&quot;')||'''';
    end if;
    return v_return;
  end;
  ...
  begin
    if 1=2 then null;
      ...
      htp.prn('javascript: var link=window.open(encodeURI(''!IMP_DEMO_FIELDS_REF.openLOV?PLOV=LOV_JOBS&FIN=IMP_DEMO_FIELDS_REF.'||name||'&PID=''+document.IMP_DEMO_FIELDS_REF.'||name||'.value+''''),''openLOV'',''resizable,scrollbars,width=680,height=550'');');
    end if;
  end;
...
</span></td><td class="rasdTxB10SAL rasdTxTypeN" id="rasdTxB10SAL_'||iB10||'">');  if B10SAL#SET(iB10).visible then  
htp.prn('<input name="B10SAL_'||iB10||'" id="B10SAL_'||iB10||'_RASD" type="text" value="'||ltrim(to_char(B10SAL(iB10),'999G990D00'))||'" class="rasdTextN');  if B10SAL#SET(iB10).error is not null then htp.p(' errorField'); end if;  
htp.prn('');  if B10SAL#SET(iB10).info is not null then htp.p(' infoField'); end if;  
htp.prn('"');  htp.p(B10SAL#SET(iB10).custom);  
htp.prn('');  if B10SAL#SET(iB10).required then htp.p(' required="required"'); end if;  
htp.prn('');  if B10SAL#SET(iB10).disabled then htp.p(' disabled="disabled"'); end if;  
htp.prn('');  if B10SAL#SET(iB10).readonly then htp.p(' readonly="readonly"'); end if;  
htp.prn('');  if B10SAL#SET(iB10).error is not null then htp.p(' title="'||B10SAL#SET(iB10).error||'"'); end if;  
htp.prn('');  if B10SAL#SET(iB10).info is not null then htp.p(' title="'||B10SAL#SET(iB10).info||'"'); end if;  
htp.prn('/>');  end if;  
htp.prn('
</td><td class="rasdTxB10SALCALC rasdTxTypeC" id="rasdTxB10SALCALC_'||iB10||'">');  if B10SALCALC#SET(iB10).visible then  
htp.prn('<input ONCLICK="js_Clov$SALCALclick(this.value, ''B10SALCALC_'||iB10||''');" name="B10SALCALC_'||iB10||'" id="B10SALCALC_'||iB10||'_RASD" type="checkbox" value="'||B10SALCALC(iB10)||'" class="rasdCheckbox');  if B10SALCALC#SET(iB10).error is not null then htp.p(' errorField'); end if;  
htp.prn('');  if B10SALCALC#SET(iB10).info is not null then htp.p(' infoField'); end if;  
htp.prn('"');  if B10SALCALC#SET(iB10).info is not null then htp.p(' title="'||B10SALCALC#SET(iB10).info||'"'); end if;  
htp.prn('');  if B10SALCALC#SET(iB10).readonly then htp.p(' readonly="readonly"'); end if;  
htp.prn('');  if B10SALCALC#SET(iB10).disabled then htp.p(' disabled="disabled"'); end if;  
htp.prn('');  if B10SALCALC#SET(iB10).required then htp.p(' required="required"'); end if;  
htp.prn('');  htp.p(B10SALCALC#SET(iB10).custom);  
htp.prn('');  if B10SALCALC#SET(iB10).error is not null then htp.p(' title="'||B10SALCALC#SET(iB10).error||'"'); end if;  
htp.prn('/><SCRIPT LANGUAGE="JavaScript"> document.addEventListener(''DOMContentLoaded'', function(event) {  js_Clov$SALCALinit( '''||B10SALCALC(iB10)||''' , ''B10SALCALC_'||iB10||''' ); }) </SCRIPT>');  end if;  
htp.prn('
...
if set_session_block__ is not null then  execute immediate set_session_block__;  end if;
    ...
    htp.p('  while(i < document.IMP_DEMO_FIELDS_REF.elements.length){');
    htp.p('    if (document.IMP_DEMO_FIELDS_REF.elements[i].name == ''B10JOB''+pi) {');
    htp.p('     document.IMP_DEMO_FIELDS_REF.elements[i].value = w.DEMO_CFORM.WEBCLIENT.B10JOB.value;}');
    htp.p('    if (document.IMP_DEMO_FIELDS_REF.elements[i].name == ''B10JOB2''+pi) {');
    htp.p('     document.IMP_DEMO_FIELDS_REF.elements[i].value = w.DEMO_CFORM.WEBCLIENT.B10JOB.value;}');
    htp.p('    i++;');
    htp.p('  }');
    htp.p('}');
...
<head>');  htpClob(rasd_client.getHtmlJSLibrary('HEAD','DEMO_FIELDS REferenced FORM')); htpClob(FORM_UIHEAD); htp.p('<style type="text/css">'); htpClob(FORM_CSS); htp.p('</style><script type="text/javascript">'); htpClob(FORM_JS); htp.p('</script>');  
htp.prn('</head>
<body><div id="IMP_DEMO_FIELDS_REF_LAB" class="rasdFormLab">DEMO_FIELDS REferenced FORM '|| rasd_client.getHtmlDataTable('IMP_DEMO_FIELDS_REF_LAB') ||'     </div><div id="IMP_DEMO_FIELDS_REF_MENU" class="rasdFormMenu">'|| rasd_client.getHtmlMenuList('IMP_DEMO_FIELDS_REF_MENU') ||'     </div>
<form name="IMP_DEMO_FIELDS_REF" method="post"><div id="IMP_DEMO_FIELDS_REF_DIV" class="rasdForm"><div id="IMP_DEMO_FIELDS_REF_HEAD" class="rasdFormHead"><input name="RECNUMB10" id="RECNUMB10_RASD" type="hidden" value="'||ltrim(to_char(RECNUMB10))||'"/>
...
</div><div id="IMP_DEMO_FIELDS_REF_BODY" class="rasdFormBody">'); output_P_DIV; htp.p(''); output_B10_DIV; htp.p(''); output_B20_DIV; htp.p(''); output_B25_DIV; htp.p(''); output_B30_DIV; htp.p('</div><div id="IMP_DEMO_FIELDS_REF_ERROR" class="rasdFormMessage error">
<font id="ERROR_RASD" class="rasdFont">'||ERROR||'</font></div><div id="IMP_DEMO_FIELDS_REF_WARNING" class="rasdFormMessage warning"><font id="WARNING_RASD" class="rasdFont">'||WARNING||'</font></div><div id="IMP_DEMO_FIELDS_REF_MESSAGE" class="rasdFormMessage"><font id="MESSAGE_RASD" class="rasdFont">'||MESSAGE||'</font></div><div id="IMP_DEMO_FIELDS_REF_FOOTER" class="rasdFormFooter">');  
if  ShowFieldGBUTTONSRC  then  
htp.prn('<input onclick=" ACTION.value=this.value; submit();" name="GBUTTONSRC" id="GBUTTONSRC_RASD" type="button" value="'||GBUTTONSRC||'" class="rasdButton"/>');  end if;  
htp.prn('
');  
...
htp.prn('<input onclick=" ACTION.value=this.value; submit();" name="GBUTTONCUSTOM" id="GBUTTONCUSTOM_RASD" type="button" value="'||GBUTTONCUSTOM||'" class="rasdButton"/>');  end if;  
htp.prn('
'|| rasd_client.getHtmlFooter(version , substr('IMP_DEMO_FIELDS_REF_FOOTER',1,instr('IMP_DEMO_FIELDS_REF_FOOTER', '_',-1)-1) , '') ||'</div></div></form></body></html>
    ');
  null; end;
...
if RESTRESTYPE = 'XML' then
    htp.p('<?xml version="1.0" encoding="UTF-8"?>'); 
    htp.p('<form name="IMP_DEMO_FIELDS_REF" version="1">'); 
    htp.p('<formfields>'); 
...
begin  
  ...
  rasd_client.secCheckPermission('IMP_DEMO_FIELDS_REF',ACTION);  
...
if RESTRESTYPE = 'XML' then
    htp.p('<?xml version="1.0" encoding="UTF-8"?>
<form name="IMP_DEMO_FIELDS_REF" version="1">');     htp.p('<error>');     htp.p('  <errorcode>'||sqlcode||'</errorcode>');     htp.p('  <errormessage>'||sqlerrm||'</errormessage>');     htp.p('</error>');     htp.p('</form>'); else
    htp.p('{"form":{"@name":"IMP_DEMO_FIELDS_REF","@version":"1",' );     htp.p('"error":{');     htp.p('  "errorcode":"'||sqlcode||'",');     htp.p('  "errormessage":"'||sqlerrm||'"');     htp.p('}');     htp.p('}}'); end if;
...
function metadata_xml return cctab is
  ...
  v_vc cctab;
  begin
 v_vc(1) := '<form><formid>71</formid><form>IMP_DEMO_FIELDS_REF</form><version>1</version><change>30.01.2020 09/28/57</change><user>RASDCLI</user><label><![CDATA[DEMO_FIELDS REferenced FORM]]></label><lobid>RASD</lobid><program></program><referenceyn>Y</referenceyn><autodeletehtmlyn>Y</autodeletehtmlyn><autocreaterestyn>Y</autocreaterestyn><autocreatebatchyn>Y</autocreatebatchyn><addmetadatainfoyn>N</addmetadatainfoyn><compiler><engineid>11</engineid><server>rasd_engine11</server><client>rasd_enginehtml11</client><library>rasd_client</library></compiler><compiledInfo><info><engineid>11</engineid><change>30.01.2020 09/28/35</change><compileyn>N</compileyn><application>imported form</application><owner>demo</owner><editor>demo</editor></info></compiledInfo><blocks></blocks><fields></fields><links></links><pages></pages><triggers></triggers><elements></elements></form>';
     return v_vc;
  end;
...
procedure metadata is
  ...
  HTP.p('Content-Disposition: filename="Export_IMP_DEMO_FIELDS_REF_v.1.1.20200130092857.xml"');
